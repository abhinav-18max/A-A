...
output = root / f"dist/mba-media-{VERSION}-linux-x86_64.tar.gz"
output.parent.mkdir(exist_ok=True)
with tempfile.TemporaryDirectory() as temp:
    manifest = Path(temp) / "runtime.json"
    manifest.write_text(json.dumps({"version": VERSION, "system": "Linux", "machine": "x86_64"}))
